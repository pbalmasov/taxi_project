...
public class MyOrderItemActivity extends BalanceActivity {
	...
	private void initActionDialog() {
		final CharSequence[] items = { (order.get_invitationtime()!=null)?this.getString(R.string.invite):"Поторопить",
	...
	private void priceDialog() {
		...
		cb.setText(MyOrderItemActivity.this.getText(R.string.end_point) + " " + order.get_addressarrival());
	...
	private Button.OnClickListener onSavePrice(final Dialog dialog) {
		return new Button.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (input.getText().length() != 0 && (cashless.getText().length() != 0 || order.get_paymenttype() == 0)) {
