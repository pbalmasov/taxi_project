...
public class MyOrderItemActivity extends BalanceActivity {
	...
	private void initOrder(Document doc) throws DOMException, ParseException {
		...
		order = new MyCostOrder(this, orderId, nominalcost, addressdeparture, carClass, comment, addressarrival,
				paymenttype, invitationtime, departuretime, accepttime, driverstate);
	...
	private OnClickListener onContextMenuItemListener() {
		return new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int item) {
				...
				// , если 1 то ПОТОРОПИТЬ, ЗАКРЫТЬ, ЗВОНОК ИЗ ОФИСА
				// если drivestate = 2, то активировать пункты ОТКАЗАТЬСЯ И
				// ЗВОНОК ИЗ ОФИСА
				// , если вдруг пришло 3, то ОТЛОЖИТЬ, ЗВОНОК ИЗ ОФИСА
				dialog.dismiss();
		...
		if (doc != null) {
			...
			if (responseNode.getTextContent().equalsIgnoreCase("failure"))
				...
				new AlertDialog.Builder(MyOrderItemActivity.this).setTitle("Звонок")
						.setMessage("Ваш запрос принят. Пожалуйста ожидайте звонка").setNeutralButton("Ок", null)
						.show();
			}
		}
	}
