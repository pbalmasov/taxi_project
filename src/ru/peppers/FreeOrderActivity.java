    ...
    private void initMainList(Document doc) throws DOMException, ParseException {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            if (type == 0) {
                ...
                orders.add(new CostOrder(date, adress, carClass, text, where, cost, costType));
            }
            if (type == 1) {
                String text = nodeList.item(i).getTextContent();
                orders.add(new NoCostOrder(date, adress, carClass, text, where));
            }
            if (type == 2) {
                String text = nodeList.item(i).getTextContent();
                orders.add(new PreliminaryOrder(date, adress, carClass, text, where));
            }
        }
